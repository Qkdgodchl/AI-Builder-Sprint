...
**잇다 ITDA (Pixel Care)**는 **VolunteerCatalog Editorial Art Magazine & Bento Grid 디자인 시스템**과 **Upstage Solar LLM AI** 기술을 결합하여, 사용자의 선행 의향을 자연어로 파싱하고 봉사·기부 약정의 체결부터 갱신, 전자서명 보관, 그리고 원하는 선행을 자유롭게 제안하고 센터가 프로그램을 개설하는 **CONNECT(온기 잇다)**까지 원스톱으로 제공하는 혁신적인 선행 커뮤니티 플랫폼입니다.
...
### 1. 🤝 CONNECT (온기 잇다 — 선행 아이디어 & 개설 제안)
- **원하는 활동 제안**: 일반 사용자가 원하는 봉사 활동이나 기부 아이디어를 자유롭게 작성하여 등록.
- **AI 대화 연동 (`connectDraft`)**: AI 챗봇(`Pixel AI Mate`)과 대화 중 원하는 미션이 있으면 즉시 CONNECT 요청 초안으로 자동 연결.
- **이웃 응원 (`Support`) & 센터 수락 (`Claim`)**: 다른 유저들이 응원 단추를 눌러 관심도를 표출하면, 센터 관리자(`CENTER_MANAGER`)가 이를 수락하여 실제 모집글 프로그램으로 개설.

### 2. 🎨 VolunteerCatalog Editorial Art Magazine & Bento Grid UI
...
### 3. 🤖 Upstage Solar LLM AI 파이프라인 (`Pixel AI Mate`)
...
### 4. 📝 CLM (Contract Lifecycle Management) & 모두싸인 전자서명
...
- **일반 사용자 (`USER`)** — `[ 🏠 홈 | 🎁 선행하기 | 💬 커뮤니티 | 📜 내 기록 ]` (4대 탭) + `CONNECT` 상단 내비게이션
...
### 8. 📊 실시간 플랫폼 종합 통계 (Platform Stats)
