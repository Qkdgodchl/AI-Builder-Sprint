...
서비스 안의 AI(Upstage)와 별개로, 기획 → 구현 → 디버깅 → 테스트 → 문서화 → 품질 감사 전 과정을
AI 코딩 에이전트(Claude Code)와 페어로 진행했습니다. 한국어 서술형 커밋 170여 개가 그 협업 기록입니다.

### 📐 에이전트 지침 체계

| 파일 | 역할 |
|---|---|
| [`CLAUDE.md`](CLAUDE.md) | 에이전트 작업 기준 — 빌드 · 테스트 명령, DB 문법 규칙, 외부 API 폴백 원칙, 트랜잭션 주의사항 |
| [`docs/AGENTS.md`](docs/AGENTS.md) | 개발 에이전트 공통 지침 — 페르소나, 권한 체계, 디자인 시스템, 폴백 규칙 |
| [`.agents/skills/pixel-roadmap-design`](.agents/skills/pixel-roadmap-design/SKILL.md) | 커스텀 스킬 — 벤토 그리드 디자인 규격을 스킬로 정의해 전 화면 UI 통일 |
| [`docs/workflow.md`](docs/workflow.md) | 서비스 기획서 — 에이전트가 기능을 구현할 때 참조하는 요구사항 원천 |

지침은 장식이 아니라 사고에서 배운 규칙입니다. 예를 들어 `CLAUDE.md`의 "MySQL 전용 문법 금지",
"트랜잭션을 잡은 채 외부 API를 기다리지 않는다"는 아래 실사례에서 실제로 겪고 지침화한 것입니다.

### 🔁 단계별 활용 방식

| 단계 | AI와 함께한 일 |
|---|---|
| 기획 · 설계 | 기획서(workflow.md) 초안과 도메인 모델 · 상태 전이 설계 검토 |
| 구현 | Upstage · 모두싸인 연동, CLM 파이프라인, 화면 개발을 지침 기반으로 페어 코딩 |
| 디버깅 | 로그 · DB를 함께 파고들어 근본 원인 확정 후 최소 수정 — 아래 사례 표 참조 |
| 테스트 | 버그마다 회귀 테스트를 함께 작성 (JUnit 55개, 수정 전후 전체 통과 확인) |
| 문서화 | 코드가 바뀔 때마다 README · docs를 코드 기준으로 동기화 |
| 품질 감사 | 제출 전 죽은 코드 · 문서 정합성 · 시크릿 노출을 에이전트로 전수 감사 |

### 🔬 AI가 실제로 기여한 사례 (커밋으로 검증 가능)

| 사례 | 커밋 |
|---|---|
| 배포 DB(PostgreSQL)에서만 터지던 MySQL 전용 `UPDATE ... JOIN` 3곳 발견 · 수정 — 승인 기능 복구 | `fd626b6` |
| 재서명한 약정서가 영영 "서명 대기"에 갇히던 상태 전이 버그 진단 (이벤트 랭크 미초기화) + 회귀 테스트 | `6083367` |
| 체결본 보관 실패가 트랜잭션을 rollback-only로 오염시켜 500을 내던 구조를 독립 트랜잭션으로 격리 | `a3355fb` |
| 커넥션 풀 고갈(외부 API를 기다리며 커넥션 점유) 원인 분석과 대기 상한 조정 | `03dd7a2` |
| 무거운 통계 API가 헬스체크로 쓰여 멀쩡한 인스턴스가 재시작되던 문제 → 경량 `/healthz` 분리 | `339a5d9` |
| 30분마다 로그아웃되던 세션을 동시 요청 안전한 토큰 재발급으로 해결 | `b3e477d` |
| 미사용 레거시 컨트롤러 · 죽은 주석 330여 줄 제거와 문서-코드 정합성 전수 감사 | `64a78e2` |
